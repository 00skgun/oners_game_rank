...
export default function Admin(){
  ...
  async function load(){if(!supabase)return;const [p,f,t,m,s,lp,ps]=await Promise.all([
  ...
  ]);setPlayers(p.data??[]);setFcMatches(f.data??[]);setTeams(t.data??[]);setLolMatches(m.data??[]);setLolSets(s.data??[]);setLolPlayers(lp.data??[]);setPlayerStats(ps.data??[]);}
