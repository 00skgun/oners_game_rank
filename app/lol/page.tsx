...
import {killParticipation,lolPlayers,playerKda,type LolPlayerStat} from '@/lib/lol-players';
import {createBrowserSupabase} from '@/lib/supabase/browser';

type PublicMatch={a:string;b:string;aw:number;bw:number;status:'완료'|'예정';playedAt:string};
...
export default function Lol(){
  ...
  const [playerRows,setPlayerRows]=useState<LolPlayerStat[]>(lolPlayers);
  useEffect(()=>{const supabase=createBrowserSupabase();if(!supabase)return;Promise.all([
    ...
    supabase.from('lol_players').select('id,team_id,real_name,nickname,tag_line,position'),
    supabase.from('lol_player_set_stats').select('player_id,kills,deaths,assists'),
  ]).then(([t,m,s,p,ps])=>{if(!t.data?.length||!m.data?.length)return;const setRows=s.data??[];const names=new Map(t.data.map(x=>[x.id,x.name]));const stats=new Map<string,LolStat>(t.data.map(x=>[x.id,{name:x.name,sets:0,kills:0,deaths:0,assists:0}]));const publicMatches:PublicMatch[]=[];for(const match of m.data){const sets=setRows.filter(x=>x.match_id===match.id);let aw=0,bw=0;for(const set of sets){if(set.winner_team_id===match.team_a_id)aw++;else if(set.winner_team_id===match.team_b_id)bw++;const a=stats.get(match.team_a_id)!,b=stats.get(match.team_b_id)!;a.sets++;b.sets++;a.kills+=set.team_a_kills;a.deaths+=set.team_b_kills;a.assists+=set.team_a_assists;b.kills+=set.team_b_kills;b.deaths+=set.team_a_kills;b.assists+=set.team_b_assists}publicMatches.push({a:names.get(match.team_a_id)??'-',b:names.get(match.team_b_id)??'-',aw,bw,status:match.status==='완료'?'완료':'예정',playedAt:match.scheduled_at?.slice(0,10)??''})}setTeams(t.data.map(x=>x.name));setMatches(publicMatches);setTeamStats([...stats.values()]);if(p.data?.length&&ps.data?.length){setPlayerRows(p.data.map(player=>{const rows=ps.data.filter(x=>x.player_id===player.id);return{team:names.get(player.team_id)??'-',position:player.position,realName:player.real_name,nickname:player.nickname,tag:player.tag_line,sets:rows.length,kills:rows.reduce((n,x)=>n+x.kills,0),deaths:rows.reduce((n,x)=>n+x.deaths,0),assists:rows.reduce((n,x)=>n+x.assists,0),teamKills:stats.get(player.team_id)?.kills??0}}))}})},[]);
  ...
  const playerStats=[...playerRows].sort((a,b)=>playerKda(b)-playerKda(a)||b.kills-a.kills||b.assists-a.assists);
  ...
  return <main className="wrap section"><div className="eyebrow">LEAGUE OF LEGENDS</div><h1>4팀 단일 풀리그</h1><p className="sub">각 팀이 서로 한 번씩 경기합니다. 풀리그 종료 후 1·2위는 결승, 3·4위는 3·4위전을 진행합니다.</p>
    ...
    <section className="section"><h2>경기 일정 및 결과</h2><div className="match-list">{matches.map(m=><div className="match" key={`${m.a}-${m.b}-${m.playedAt}`}><span className="state">{m.playedAt||'일정 미정'}</span><b className="right">{m.a}</b><span className="score">{m.status==='완료'?`${m.aw} : ${m.bw}`:'VS'}</span><b>{m.b}</b><span className="badge">{m.status}</span></div>)}</div></section>
